...
/**
 * Filename used on the mount, keyed by infoHash AND fileIdx — not infoHash
 * alone. A season pack is one infoHash with many files; keying on infoHash
 * only made a mount hit for ANY episode of a show serve whichever episode
 * happened to be mounted first, regardless of which one was actually
 * requested (found live: browsing to episode 5 silently played episode 1).
 * fileIdx disambiguates which file within the torrent this is.
 */
function mountFilename(infoHash, originalPath, fileIdx) {
  const match = /\.[a-z0-9]+$/i.exec(String(originalPath || ''));
  const ext = match ? match[0] : '.mkv';
  return `${String(infoHash).toLowerCase()}.${fileIdx}${ext}`;
}
