...
/** Google's Range-capable direct media endpoint — skips the large-file
 * virus-scan warning page a plain drive.google.com share link hits. */
function driveMediaUrl(fileId) {
  return `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media&key=${encodeURIComponent(config.LOCALSEED.GDRIVE_API_KEY)}`;
}
...
async function moveToMount(torrent, infoHash, title, wantedIdx) {
  ...
  // Only bother making the file publicly link-shareable if a direct-stream
  // API key is actually configured — no point exposing it otherwise.
