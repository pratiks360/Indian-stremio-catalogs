...
async function streamRelease(req, res, release, season, episode) {
  ...
  if (mounted) {
    ...
    if (config.LOCALSEED.GDRIVE_API_KEY && entry.driveFileId) {
      activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'serve-from-drive-direct', success: true });
...
async function moveToMount(torrent, infoHash, title, wantedIdx) {
  ...
  if (config.LOCALSEED.GDRIVE_API_KEY) {
    ...
    if (fileId) {
      ...
      activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'drive-share', success: true });
    } else {
      activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'drive-share', success: false, errorMsg: 'rclone link failed' });
    }
  }
  ...
  if (cleaned.length) {
    activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'cleanup-siblings', success: true, errorMsg: `${cleaned.length} file(s)` });
  }
