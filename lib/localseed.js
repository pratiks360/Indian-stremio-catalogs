...
async function streamRelease(req, res, release, season, episode) {
  ...
  if (!torrent) {
    ...
    // A season pack contains every episode; WebTorrent downloads every file
    // in a torrent by default. Deselect them all now — only the specific
    // file pickFileIdx() chooses below gets selected (and therefore
    // downloaded), so requesting one episode never pulls the whole season.
  ...
  // Record which file this torrent is actually for, so moveToMount() moves
  // the requested episode — not a "largest file" guess that picks the
  // wrong episode in a season pack. Last-requested-episode wins if the
  // same in-progress torrent is later asked for a different episode; that
  // combination is rare enough to accept as a known limitation.
  torrent._wantedIdx = fileIdx;
  file.select();
...
async function moveToMount(torrent, infoHash, title) {
  ...
  // Move the file that was actually requested (tracked in streamRelease),
  // not a "largest file" guess — in a season pack the largest episode is
  // rarely the one the user asked for. Falls back to largest only if
  // _wantedIdx was somehow never set (defensive; should not happen given
  // streamRelease always sets it before a torrent can reach 'done').
  const wantedIdx = torrent._wantedIdx != null
    ? torrent._wantedIdx
    : torrent.files.reduce((bestIdx, f, i, arr) => (f.length > arr[bestIdx].length ? i : bestIdx), 0);
  ...
  const destPath = path.join(destDir, mountFilename(infoHash, wanted.path));

  const t0 = Date.now();
  await fs.promises.copyFile(srcPath, destPath);
  await fs.promises.unlink(srcPath);

  state.touch(STATE_PATH, infoHash, { size: wanted.length, lastPlayed: Date.now(), status: 'mounted', releaseTitle: title });
