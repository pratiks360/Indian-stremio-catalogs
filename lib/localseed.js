...
// The "VPS/Drive Downloaded" catalog (lib/localseed_catalog.js) reads this
// sidecar through a 5-minute TTL cache — fine for steady-state, but a user
// who just started or finished a local-seed download expects to see it on
// the catalog right away, not after up to 5 minutes. Every write below that
// changes what the catalog would show (new download starts, moves to the
// mount, or gets evicted) clears the cache so the next request rebuilds
// fresh instead of waiting out the TTL.
function invalidateCatalogCache() {
  cache.clear('catalog:localseed');
}

const STATE_PATH = path.join(__dirname, '..', 'data', 'localseed_meta.json');

// Must match the remote name deploy/rclone-gdrive-mount.service mounts
...
/**
 * Snapshot of one live torrent's peer-seeding counters, in the tracker-ratio
 * sense: bytes uploaded to other torrent peers (torrent.uploaded — HTTP
 * serving to the user's own player does NOT count, only wire-protocol
 * uploads to peers), plus the share of verified downloaded bytes that has
 * been re-seeded. Null when there is no live torrent, e.g. a mount serve
 * long after the seed window ended — the activity-log row then renders '—'.
 * @param {object|null} torrent webtorrent torrent object
 * @returns {null|{uploadedBytes:number, downloadedBytes:number, ratioPct:number|null}}
 */
function seedSnapshot(torrent) {
  if (!torrent) return null;
  const uploadedBytes = torrent.uploaded || 0;
  const downloadedBytes = torrent.downloaded || 0;
  return {
    uploadedBytes,
    downloadedBytes,
    ratioPct: downloadedBytes > 0 ? Math.round((uploadedBytes / downloadedBytes) * 100) : null
  };
}

/** seedSnapshot() for the active torrent matching this infoHash, if any. */
function seedSnapshotFor(infoHash) {
  return seedSnapshot(findActiveTorrent(infoHash));
}

/**
 * Flatten a snapshot into the fields the activity log stores — spreads to
 * nothing when there is no live torrent, so the log row simply lacks the
 * fields and the page renders '—' rather than inventing a zero.
 */
function seedFields(snap) {
  return snap
    ? { uploadedBytes: snap.uploadedBytes, downloadedBytes: snap.downloadedBytes, ratioPct: snap.ratioPct }
    : {};
}
...
async function streamRelease(req, res, release, season, episode) {
  ...
  if (mounted) {
    const key = stateKey(infoHash, wantedIdx);
    const entry = state.load(STATE_PATH)[key] || {};
    state.touch(STATE_PATH, key, { lastPlayed: Date.now() });

    // A shared file streams straight from Google's servers instead of
    // being proxied through this VPS — only when GDRIVE_API_KEY is set
    // (see config.js) and this specific file was actually shared (moved
    // in before the key existed, or the share attempt failed — either
    // way, driveFileId absent just means fall back to the VPS proxy).
    if (config.LOCALSEED.GDRIVE_API_KEY && entry.driveFileId) {
      activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'serve-from-drive-direct', success: true, ...seedFields(seedSnapshotFor(infoHash)) });
      return res.redirect(302, driveMediaUrl(entry.driveFileId));
    }

    activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'serve-from-mount', success: true, ...seedFields(seedSnapshotFor(infoHash)) });
  ...
  if (!torrent) {
    ...
    // known limitation — the bandwidth cost and the longer wait before the
    // wanted file reaches the mount both still apply. What moveToMount()
    // below does clean up is the DISK cost: the unwanted sibling episodes
    // are deleted once the wanted one is safely moved, rather than sitting
    // on local disk forever (confirmed live: a single un-swept season pack
    // left 4.5GB of never-requested episodes behind on a 45GB box).
    activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'download-start', success: true, ...seedFields(seedSnapshot(torrent)) });

    torrent.once('done', () => {
      moveToMount(torrent, infoHash, title, wantedIdx).catch(err => {
        activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'move-to-mount', success: false, errorMsg: err.message, ...seedFields(seedSnapshot(torrent)) });
      });
    });
  }
...
/**
 * Makes a mounted file "anyone with link" shareable via rclone and returns
 * its Drive file id (parsed out of the share URL rclone prints), or null on
 * any failure — a share failure is never fatal to playback, callers just
 * keep proxying that file through the VPS instead.
 * @param {string} mountRelativePath filename as it sits under mountDir(),
 *   e.g. "abc123.0.mkv"
 */
function shareOnDrive(mountRelativePath) {
  return new Promise(resolve => {
    const remotePath = `${RCLONE_REMOTE}:${config.LOCALSEED.MOUNT_SUBDIR}/${mountRelativePath}`;
    execFile('rclone', ['link', remotePath], { timeout: 30000 }, (err, stdout) => {
      if (err) { resolve(null); return; }
      const m = /[?&]id=([^&\s]+)/.exec(stdout);
      resolve(m ? m[1] : null);
    });
  });
}
...
/**
 * Direct media URL for a publicly-shared Drive file — Range-capable (302
 * verified live: 206 + correct Content-Range), and skips the large-file
 * virus-scan warning page a plain drive.google.com/open?id= share link
 * hits. NOT the documented googleapis.com/drive/v3/files?alt=media
 * endpoint — that one is blocked for bare-API-key auth as of 2026
 * (confirmed live: 403 API_KEY_SERVICE_BLOCKED, Google requires OAuth for
 * it now). This is drive.usercontent.google.com's own download path,
 * undocumented but currently working; Google has changed this pattern
 * before without notice, so a failure here should just fall back to the
 * VPS proxy rather than erroring the whole stream.
 */
...
async function moveToMount(torrent, infoHash, title, wantedIdx) {
  if (!mountReady()) {
    activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'move-to-mount', success: false, errorMsg: 'mount not available', ...seedFields(seedSnapshot(torrent)) });
    return;
  }
  ...
  const key = stateKey(infoHash, wantedIdx);
  state.touch(STATE_PATH, key, { size: wanted.length, lastPlayed: Date.now(), status: 'mounted', releaseTitle: title });
  invalidateCatalogCache();
  activityLog.localSeed({
    infoHash, releaseTitle: title, phase: 'move-to-mount', success: true, duration_ms: Date.now() - t0,
    ...seedFields(seedSnapshot(torrent))
  });

  // Only bother making the file publicly link-shareable if direct-from-
  // Drive streaming is actually enabled (GDRIVE_API_KEY set — see
  // config.js) — no point exposing it otherwise.
  if (config.LOCALSEED.GDRIVE_API_KEY) {
    const fileId = await shareOnDrive(path.basename(destPath));
    if (fileId) {
      state.touch(STATE_PATH, key, { driveFileId: fileId });
      activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'drive-share', success: true, ...seedFields(seedSnapshot(torrent)) });
    } else {
      activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'drive-share', success: false, errorMsg: 'rclone link failed', ...seedFields(seedSnapshot(torrent)) });
    }
  }

  // Season packs download every episode (see the NOTE above), not just the
  // one requested. The wanted file is already off local disk (unlinked
  // above); everything else in this torrent's own download folder is a
  // never-requested sibling episode that will just sit there forever
  // unless removed here — confirmed live: one un-swept pack left 4.5GB of
  // dead weight on a 45GB box. Scoped to files actually belonging to this
  // torrent (torrent.files), so it can never touch another download.
  const cleaned = [];
  for (let i = 0; i < torrent.files.length; i++) {
    if (i === wantedIdx) continue;
    const siblingPath = path.join(config.LOCALSEED.LOCAL_DIR, torrent.files[i].path);
    try {
      await fs.promises.unlink(siblingPath);
      cleaned.push(siblingPath);
    } catch { /* already gone, or never fully written — fine either way */ }
  }
  if (cleaned.length) {
    activityLog.localSeed({ infoHash, releaseTitle: title, phase: 'cleanup-siblings', success: true, errorMsg: `${cleaned.length} file(s)`, ...seedFields(seedSnapshot(torrent)) });
  }
  ...
  setTimeout(() => {
    // Pin down the torrent's FINAL cumulative seeding totals before the
    // client drops it — uploaded keeps climbing while announcing during
    // SEED_WINDOW_MS, and once the torrent is gone no later event (a mount
    // serve, an eviction) can read the counters. This event is the log's
    // authoritative per-release "how much % and MB was seeded" answer.
    const snap = seedSnapshot(torrent);
    getClient().remove(torrent.infoHash, () => {});
    activityLog.localSeed({
      infoHash, releaseTitle: title, phase: 'seed-window-end', success: true, ...seedFields(snap)
    });
...
async function sweepEviction() {
  ...
  for (const target of targets) {
    try {
      ...
      invalidateCatalogCache();
      activityLog.localSeed({ infoHash: target.infoHash, phase: 'evict', success: true, ...seedFields(seedSnapshotFor(target.infoHash)) });
    } catch (err) {
      activityLog.localSeed({ infoHash: target.infoHash, phase: 'evict', success: false, errorMsg: err.message, ...seedFields(seedSnapshotFor(target.infoHash)) });
    }
  }
}

/**
 * Cancel + wipe a still-downloading VPS torrent (from the /activity "Files"
 * delete action). Destroys the WebTorrent handle AND its on-disk pieces
 * (destroyStore), then drops every composite state entry for this infoHash
 * (a season pack can have several — same reasoning as listActive()'s prefix
 * match).
 * @param {string} infoHash
 * @returns {Promise<boolean>} true if a matching active torrent was found
 */
function deleteActive(infoHash) {
  return new Promise(resolve => {
    const t = findActiveTorrent(infoHash);
    if (!t) { resolve(false); return; }
    getClient().remove(t, { destroyStore: true }, err => {
      if (err) console.warn(`[localseed] deleteActive remove failed: ${err.message}`);
      const meta = state.load(STATE_PATH);
      const prefix = `${String(infoHash).toLowerCase()}.`;
      for (const k of Object.keys(meta)) {
        if (k.startsWith(prefix)) state.remove(STATE_PATH, k);
      }
      invalidateCatalogCache();
      resolve(true);
    });
...
/**
 * Delete one file off the Google Drive mount (from the /activity "Files"
 * delete action). `fileName` is exactly what listMounted() returns as
 * `.file` — a bare basename, never a path, so it is validated as such
 * before touching the filesystem.
 * @param {string} fileName
 * @returns {boolean} true (idempotent — already-gone is not an error)
 */
function deleteMounted(fileName) {
  if (!fileName || /[\\/]/.test(fileName) || fileName.includes('..')) {
    throw new Error(`invalid mounted file name: ${fileName}`);
  }
  const full = path.join(mountDir(), fileName);
  try {
    fs.unlinkSync(full);
  } catch (err) {
  ...
  for (const k of Object.keys(meta)) {
    if (lower.startsWith(k.toLowerCase())) state.remove(STATE_PATH, k);
  }
  invalidateCatalogCache();
  return true;
}
...
module.exports = {
  ...
  deleteActive, deleteMounted, seedSnapshot, seedSnapshotFor
};
